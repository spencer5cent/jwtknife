...
JWTKnife is under active development. Known limitations include:

• JWTs passed via GET or POST parameters are not yet supported (header and cookie injection only)
• Encrypted JWTs (JWE, e.g. alg=dir, enc=A128CBC-HS256) are not currently supported
• No non-interactive / full CLI mode yet (wizard-driven only)
• External tooling (e.g. sig2n via Docker) is required for advanced algorithm confusion without exposed keys
• Error messages for unsupported JWT formats may be brief

These are intentional design trade-offs for correctness and safety, and are planned for future improvement.
